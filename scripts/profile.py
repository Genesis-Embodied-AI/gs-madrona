...
def profile_madrona(bench_cmd, block_config=range(1, 7), cache="/tmp/madcache"):
    try:
        # os.remove(cache)
        pass
    except:
...
def parse_traces(block_config=range(1, 7)):
    ...
    for i in block_config:
        path_to_trace = "/tmp/profile_{}_block_madrona_device_tracing.bin".format(i)
...
def generate_json(block_config=range(1, 7)):
    ...
    base_config, base_duration = min(overall_durations.items(), key=lambda x: x[1])
